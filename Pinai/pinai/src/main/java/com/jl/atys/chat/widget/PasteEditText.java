 ...
 */
package com.jl.atys.chat.widget;
...
public class PasteEditText extends EditText {
    ...
    public boolean onTextContextMenuItem(int id) {
        if (id == android.R.id.paste) {
            ...
            if (text.startsWith(AtyChat.COPY_IMAGE)) {
//                intent.setDataAndType(Uri.fromFile(new File("/sdcard/mn1.jpg")), "image/*");     
                text = text.replace(AtyChat.COPY_IMAGE, "");
                Intent intent = new Intent(context, ChatAlertDialog.class);
                intent.putExtra("title", "发送以下图片？");
                intent.putExtra("forwardImage", text);
                intent.putExtra("cancel", true);
                ((Activity) context).startActivityForResult(intent, AtyChat.REQUEST_CODE_COPY_AND_PASTE);
//                clip.setText("");
            }
        }
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        if (!TextUtils.isEmpty(text) && text.toString().startsWith(AtyChat.COPY_IMAGE)) {
            setText("");
        }
