...
public class AtyDsgyUnReadContent extends AtySupport implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       //showToast(postid + "..." + commentid);
        new ForumGetReply(postid, commentid, new ForumGetReply.SuccessCallback() {
            @Override
   ...
   // private String userNick ;
    ...
    public void initData(GetPostChildBean gc) {
        ImageView portrait = (ImageView) findViewById(R.id.comment_portrait);
        ImageView sex = (ImageView) findViewById(R.id.comment_sex);
        TextView nickname = (TextView) findViewById(R.id.comment_nickname);
        TextView creatAt = (TextView) findViewById(R.id.comment_created_at);
        TextView content = (TextView) findViewById(R.id.comment_content);
        LinearLayout replyl = (LinearLayout) findViewById(R.id.comment_reply_content);

        UserTools.displayImage(gc.getUserPortrait(), portrait, getOptions());
        if (gc.getUserSex().equals("F")) {
            sex.setBackgroundResource(R.drawable.big_girl);
        } else {
            sex.setBackgroundResource(R.drawable.big_boy);
        }
       // userNick =gc.getNickname();
        nickname.setText(gc.getNickname());
        creatAt.setText(gc.getCreatedAt());
        content.setText(gc.getContent());
        List<GetPostReplyBean> re = gc.getReplyList();
        for (int i = 0; i < re.size(); i++) {
            final GetPostReplyBean gr = re.get(i);
            TextView c = new TextView(context);
            c.setText(getTwoColorText(gr.getContent()));
            c.setTextSize(14);
            c.setTextColor(R.color.gray);
            TextView t = new TextView(context);
            t.setTextSize(14);
            t.setText(gr.getCreatAt());
            t.setTextColor(R.color.gray);
            c.setOnClickListener(new View.OnClickListener( ) {
                @Override
                public void onClick(View v) {
                    try{
                        userNick =  gr.getContent().split(" 回复 ")[0];
                        mEditTextContent.setHint("回复 : "+userNick);
                    }catch (Exception e){
    ...
    private SpannableStringBuilder getTwoColorText(String str1) {
        ...
        if (length != -1) {
            style.setSpan(new ForegroundColorSpan(getResources().getColor(R.color.background_pink)), 0, length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        return style;
    }
    ...
    public void sendMsg(View v) {
        ...
        String content= Config.getCacheNickName(context) + " 回复 " + userNick + ":"+mEditTextContent.getText().toString();
        //showToast(content);
        showProgressDialog(context, "", "回复中...", false);
        String userId = Config.getCacheID(context);
        new ForumPostcomment(ForumPostcomment.TYPE_REPLY, content, userId, postid, userId, commentid, new ForumPostcomment.SuccessCallback() {
            @Override
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                try {
                    ...
                    if (filename != "delete_expression") { // 不是删除键，显示表情
                        ...
                        mEditTextContent.append(SmileUtils.getSmiledText(AtyDsgyUnReadContent.this, (String) field.get(null), 26));
                    } else { // 删除文字或者表情
